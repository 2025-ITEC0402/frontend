...
* **합성함수의 정의역:** f와 t의 합성함수의 정의역은 t의 정의역과 f의 정의역의 교집합 중에서 t(x)가 f의 정의역에 포함되는 x의 집합입니다. 즉, $$\{\,x \mid x \in (A \cap B)\ \text{and}\ t(x) \in A\}\,$$입니다. (A는 f의 정의역, B는 t의 정의역) 
...
일반적으로 함수값이 특정 값으로 수렴하지 않을 때 극한이 존재하지 않습니다. 예를 들어, $x=2$에서 좌극한과 우극한이 다르거나, 무한 불연속인 경우 극한이 존재하지 않습니다. 
...
* **(c) 상수배 법칙 (Constant Multiple Law)** 
